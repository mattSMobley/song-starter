...
const TABS = ['Generate', 'Chords', 'Session', 'Browse']
...
export default function App() {
  ...
  // ── Load persisted loops on mount ─────────────────────────────────────────
  useEffect(() => {
    loadLoops().then(saved => {
      if (saved.length) {
        setLoops(saved)
        loopCounterRef.current = saved.length
      }
    })
  }, [])
  ...
  const handleTapTempo = useCallback(() => {
    ...
    if (recent.length >= 2) {
      ...
      const newBpm = Math.round(Math.min(240, Math.max(40, 60000 / avgInterval)))
      setBpm(newBpm)
      setTempo(newBpm)
    }
  ...
  function handleMidiOutputChange(id) { setMidiOutputId(id); selectOutput(id) }
  function handleMidiChannelChange(ch) { setMidiCh(ch); setMidiChannel(ch) }
  ...
  function generateMelodies() {
    setGenerating(true)
    setTimeout(() => {
      const variations = generateVariations(root, scale, genCount, octave, genBars, activeProg)
  ...
  function handleRenameLoop(id, name) {
    setLoops(prev => prev.map(l => l.id === id ? { ...l, name } : l))
    updateLoop(id, { name })
  }

  // ── Chord progression context for generation ───────────────────────────────
  function handleChordProgChange(chords) {
    setChordHighlights(chords)
  }

  function handleActiveProgChange(prog, beatsPerChord) {
    setActiveProg(prog ? { chords: prog.chords, beatsPerChord } : null)
  }
  ...
  if (!started) {
    ...
    const FEATURES = [
      { icon:'🎹', label:'7 Instruments' },
      { icon:'🎼', label:'10 Scales' },
      { icon:'⚡', label:'Instant Generation' },
      { icon:'💾', label:'Saved to Browser' },
      { icon:'🎤', label:'Vocal Autotune' },
    ]
    ...
    return (
      <div className="relative flex items-center justify-center overflow-hidden"
        ...
        <div className="absolute inset-0 pointer-events-none" style={{
          background: `linear-gradient(to bottom, rgba(109,40,217,0.25) 0%, transparent 12%),
        ...
        {NOTE_SPAWNS.map((n, i) => (
          <div key={i} className="music-note"
            style={{ left: n.left, bottom: n.bottom, color: n.color, animationDuration: n.dur, animationDelay: n.delay,
              textShadow: `0 0 12px ${n.color}, 0 0 24px ${n.color}88`, filter: `drop-shadow(0 0 6px ${n.color})` }}>
            {NOTES[i % NOTES.length]}
          </div>
        ))}
        <div className="relative flex flex-col items-center text-center" style={{ gap: 36, zIndex: 10, padding: '0 24px' }}>
          <div className="reveal-up" style={{ animationDelay: '0.05s' }}>
            <div className="relative flex items-center justify-center" style={{ width: 96, height: 96 }}>
              <div className="absolute inset-0 rounded-full" style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.45) 0%, transparent 70%)', filter: 'blur(16px)', animation: 'halo-pulse 2.5s ease-in-out infinite' }} />
              <div className="absolute inset-0 rounded-full" style={{ border: '1px solid rgba(124,58,237,0.35)', animation: 'orbit-spin 12s linear infinite', backgroundImage: 'conic-gradient(from 0deg, transparent 80%, rgba(168,85,247,0.8) 90%, transparent 100%)' }} />
              <div className="absolute rounded-full" style={{ inset: 12, border: '1px solid rgba(6,182,212,0.3)', animation: 'orbit-spin-r 8s linear infinite', backgroundImage: 'conic-gradient(from 180deg, transparent 75%, rgba(6,182,212,0.7) 88%, transparent 100%)' }} />
              <svg width="52" height="52" viewBox="0 0 48 48" fill="none" className="relative">
                <path d="M10 34 Q16 10 24 22 Q32 34 38 12" stroke="url(#sw)" strokeWidth="2.8" fill="none" strokeLinecap="round" strokeDasharray="200" style={{ animation: 'path-draw 1.6s cubic-bezier(0.16,1,0.3,1) forwards' }} />
                <circle cx="10" cy="34" r="4" fill="#7c3aed" style={{ filter: 'drop-shadow(0 0 6px #7c3aed)' }}/>
                <circle cx="38" cy="12" r="4" fill="#06b6d4" style={{ filter: 'drop-shadow(0 0 6px #06b6d4)' }}/>
                <defs><linearGradient id="sw" x1="0" y1="0" x2="48" y2="0"><stop offset="0%" stopColor="#a855f7"/><stop offset="50%" stopColor="#818cf8"/><stop offset="100%" stopColor="#06b6d4"/></linearGradient></defs>
              </svg>
            </div>
          </div>
          <div className="reveal-up flex flex-col items-center gap-3" style={{ animationDelay: '0.18s' }}>
            <h1 className="title-gradient" style={{ fontSize: 'clamp(3.5rem, 8vw, 6.5rem)', fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1.1, paddingBottom: '0.05em' }}>
              Song Starter
            </h1>
            <div className="flex items-center gap-3" style={{ fontSize: '0.72rem', letterSpacing: '0.28em', fontWeight: 600 }}>
              {['MELODY','·','INSPIRATION','·','CREATION'].map((word, i) => (
                <span key={i} style={{
                  color: i % 2 === 1 ? 'rgba(71,85,105,0.5)' : ['#a855f7','#06b6d4','#ec4899'][Math.floor(i/2)],
                  textShadow: i % 2 !== 1 ? `0 0 16px ${['rgba(168,85,247,0.6)','rgba(6,182,212,0.6)','rgba(236,72,153,0.6)'][Math.floor(i/2)]}` : 'none',
                  animation: 'char-in 0.5s ease forwards', animationDelay: `${0.4 + i * 0.08}s`, opacity: 0,
                }}>{word}</span>
              ))}
            </div>
          </div>
          <div className="reveal-up flex flex-wrap justify-center gap-2" style={{ animationDelay: '0.32s', maxWidth: 520 }}>
            {FEATURES.map(f => (
              <div key={f.label} className="flex items-center gap-2 rounded-full"
                style={{ padding: '8px 18px', background: 'rgba(20,14,36,0.85)', border: '1px solid rgba(124,58,237,0.35)', boxShadow: '0 0 12px rgba(124,58,237,0.12), inset 0 1px 0 rgba(255,255,255,0.06)', backdropFilter: 'blur(8px)' }}>
                <span style={{ fontSize: 14 }}>{f.icon}</span>
                <span style={{ fontSize: '0.72rem', color: 'rgba(196,181,253,0.8)', fontWeight: 500, letterSpacing: '0.03em' }}>{f.label}</span>
              </div>
            ))}
          </div>
          <div className="reveal-up relative flex items-center justify-center" style={{ animationDelay: '0.44s', borderRadius: 16 }}>
            <div className="btn-ring" /><div className="btn-ring btn-ring-2" /><div className="btn-ring btn-ring-3" />
            <button onClick={handleStart} className="btn-breathe relative"
              style={{ padding: '16px 64px', borderRadius: 16, fontSize: '1.1rem', fontWeight: 700, letterSpacing: '0.06em', textTransform: 'uppercase', background: 'linear-gradient(135deg, #7c3aed 0%, #6d28d9 50%, #5b21b6 100%)', color: '#f5eeff', border: '1px solid rgba(196,132,252,0.4)', cursor: 'pointer', transition: 'transform 0.12s ease, filter 0.12s ease' }}
          ...
          <div className="reveal-up flex items-center gap-4" style={{ animationDelay: '0.56s' }}>
            {['No account needed', 'Works offline', '100% in-browser'].map((t, i) => (
              <span key={t} className="flex items-center gap-4">
                ...
                <span style={{ color: 'rgba(100,116,139,0.55)', fontSize: '0.7rem', letterSpacing: '0.05em' }}>{t}</span>
              </span>
            ))}
          </div>
        </div>
  ...
  // ── Shared style tokens ────────────────────────────────────────────────────
  const S = {
    sectionLabel: { fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase', color: 'rgba(192,132,252,0.85)', borderLeft: '3px solid rgba(168,85,247,0.8)', paddingLeft: 10, marginBottom: 14, display: 'block', textShadow: '0 0 14px rgba(168,85,247,0.5)' },
    divider:      { height: 1, background: 'linear-gradient(90deg, rgba(168,85,247,0.4), rgba(6,182,212,0.15), transparent)', margin: '4px 0' },
  }
  ...
  return (
    <div className="grid-bg flex flex-col" style={{ height: '100vh', background: '#06060c', overflow: 'hidden' }}>
      ...
      <header className="flex items-center flex-shrink-0"
        style={{ height: 60, background: 'rgba(8,4,18,0.97)', backdropFilter: 'blur(20px)', boxShadow: '0 1px 0 rgba(124,58,237,0.3), 0 0 50px rgba(0,0,0,0.6), 0 4px 24px rgba(124,58,237,0.06)' }}>
        ...
        <div className="flex items-center gap-3 flex-shrink-0"
          style={{ width: isMobile ? 'auto' : 280, paddingLeft: isMobile ? 8 : 28, paddingRight: 20, borderRight: isMobile ? 'none' : '1px solid rgba(124,58,237,0.12)', height: '100%' }}>
          <div className="relative flex items-center justify-center" style={{ width: 34, height: 34 }}>
            <div className="absolute inset-0 rounded-full" style={{ background: 'radial-gradient(circle, rgba(124,58,237,0.4) 0%, transparent 70%)', filter: 'blur(7px)' }} />
            <div className="absolute inset-0 rounded-full" style={{ border: '1px solid rgba(124,58,237,0.32)', animation: 'orbit-spin 12s linear infinite', backgroundImage: 'conic-gradient(from 0deg, transparent 80%, rgba(168,85,247,0.75) 92%, transparent 100%)' }} />
            <svg width="19" height="19" viewBox="0 0 48 48" fill="none" className="relative">
              <path d="M10 34 Q16 10 24 22 Q32 34 38 12" stroke="url(#hw)" strokeWidth="3" fill="none" strokeLinecap="round"/>
              <circle cx="10" cy="34" r="4" fill="#7c3aed"/><circle cx="38" cy="12" r="4" fill="#06b6d4"/>
              <defs><linearGradient id="hw" x1="0" y1="0" x2="48" y2="0"><stop offset="0%" stopColor="#a855f7"/><stop offset="100%" stopColor="#06b6d4"/></linearGradient></defs>
            </svg>
          </div>
          <span className="title-gradient" style={{ fontSize: '1.05rem', fontWeight: 800, letterSpacing: '-0.02em' }}>Song Starter</span>
        </div>

        {!isMobile && (
          <div className="flex-1 flex items-center justify-center">
            <div style={{ width: '100%', maxWidth: 480 }}><Visualizer /></div>
          </div>
        )}
        {isMobile && <div className="flex-1" />}

        {/* BPM + Tap + Metronome */}
        <div className="flex items-center gap-2 flex-shrink-0" style={{ marginRight: isMobile ? 12 : 28 }}>
          <div className="flex items-center gap-2 rounded-xl"
            style={{ padding: '8px 12px', background: 'rgba(14,8,28,0.95)', border: '1px solid rgba(124,58,237,0.4)', boxShadow: '0 0 22px rgba(124,58,237,0.2), inset 0 1px 0 rgba(255,255,255,0.05)' }}>
            ...
            <input
              ...
              onChange={e => { const v = parseInt(e.target.value) || 120; setBpm(v); setTempo(v); if (metronomeOn) setMetronome(true) }}
              className="w-12 text-center font-mono font-bold bg-transparent outline-none"
              style={{ fontSize: '0.95rem', color: '#c084fc' }}
            />
            <button onPointerDown={handleTapTempo}
              className="rounded-lg font-bold select-none transition-all"
              style={{ fontSize: '0.6rem', letterSpacing: '0.12em', textTransform: 'uppercase', padding: '4px 8px',
                background: tapFlash ? 'linear-gradient(135deg, rgba(168,85,247,0.55), rgba(124,58,237,0.45))' : 'rgba(124,58,237,0.15)',
                border: tapFlash ? '1px solid rgba(192,132,252,0.7)' : '1px solid rgba(124,58,237,0.3)',
                color: tapFlash ? '#e9d5ff' : 'rgba(168,85,247,0.7)',
                boxShadow: tapFlash ? '0 0 12px rgba(168,85,247,0.45)' : 'none', cursor: 'pointer' }}>
              TAP
            </button>
          </div>
          {/* Metronome toggle */}
          <button
            ...
            style={{ width: 38, height: 38,
              background: metronomeOn ? 'linear-gradient(135deg, rgba(34,211,238,0.35), rgba(6,182,212,0.2))' : 'rgba(255,255,255,0.04)',
              border: metronomeOn ? '1px solid rgba(34,211,238,0.55)' : '1px solid rgba(255,255,255,0.1)',
              color: metronomeOn ? '#22d3ee' : 'rgba(148,163,184,0.35)',
              boxShadow: metronomeOn ? '0 0 14px rgba(34,211,238,0.3)' : 'none' }}>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="flex flex-col gap-5 flex-shrink-0 overflow-y-auto"
          ...
          <section>
            ...
            <div className="flex flex-col gap-2">
              {INSTRUMENTS.map(ins => {
                ...
                return (
                  <button key={ins.id} onClick={() => { handleInstrumentChange(ins.id); if (isMobile) setSidebarOpen(false) }}
                    className="flex items-center gap-3 rounded-xl text-sm font-semibold transition-all text-left"
                    style={{
                      padding: '14px 18px',
                      background: on ? 'linear-gradient(90deg, rgba(124,58,237,0.42) 0%, rgba(124,58,237,0.1) 100%)' : 'rgba(255,255,255,0.025)',
                    ...
                    {ins.label}
                    {on && <div className="ml-auto rounded-full" style={{ width: 7, height: 7, background: '#a855f7', boxShadow: '0 0 8px #a855f7, 0 0 16px rgba(168,85,247,0.6)' }} />}
                  </button>
                )
              })}
            </div>
              ...
              {SCALE_NAMES.map(s => {
                ...
                return (
                  <button key={s} onClick={() => setScale(s)}
                    ...
                    style={{
                      padding: '11px 16px',
                      background: on ? 'linear-gradient(90deg, rgba(6,182,212,0.25) 0%, rgba(6,182,212,0.08) 100%)' : 'rgba(255,255,255,0.025)',
            ...
            <div className="flex items-center gap-2">
              {[['−', handleOctaveDown], ['+', handleOctaveUp]].map(([label, fn], i) => (
                <button key={label} onClick={fn}
                  ...
                  style={{ width: 46, height: 46, background: 'rgba(124,58,237,0.18)', border: '1px solid rgba(124,58,237,0.45)', color: '#c084fc', fontSize: '1.3rem', boxShadow: '0 0 16px rgba(124,58,237,0.22)', order: i === 0 ? 0 : 2 }}>
                  {label}
                </button>
              ))}
          ...
          <section>
            <label style={S.sectionLabel}>
              MIDI Out
              {midiOutputId && <span style={{ marginLeft: 8, display: 'inline-block', width: 6, height: 6, borderRadius: '50%', background: '#22d3ee', boxShadow: '0 0 6px #22d3ee', verticalAlign: 'middle' }} />}
            </label>
            {!midiReady ? (
              ...
              <p style={{ fontSize: '0.65rem', color: 'rgba(148,163,184,0.4)', lineHeight: 1.5 }}>
                No MIDI devices found.<br/>Enable <span style={{ color: '#c084fc' }}>IAC Driver</span> in Audio MIDI Setup.
              </p>
            ) : (
              <div className="flex flex-col gap-2">
                <select value={midiOutputId} onChange={e => handleMidiOutputChange(e.target.value)}
                  className="rounded-lg text-xs font-semibold w-full"
                  style={{ padding: '9px 12px', background: 'rgba(255,255,255,0.04)', border: midiOutputId ? '1px solid rgba(34,211,238,0.45)' : '1px solid rgba(255,255,255,0.08)', color: midiOutputId ? '#67e8f9' : 'rgba(148,163,184,0.5)', outline: 'none', cursor: 'pointer' }}>
                ...
                {midiOutputId && (
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      value={midiCh}
                      onChange={e => handleMidiChannelChange(Number(e.target.value))}
                      className="rounded-md font-mono font-bold transition-all flex-1"
                      style={{ padding: '5px 8px', fontSize: '0.72rem', background: 'rgba(34,211,238,0.12)', border: '1px solid rgba(34,211,238,0.35)', color: '#22d3ee', outline: 'none', cursor: 'pointer' }}>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-shrink-0"
            style={{
              padding: isMobile ? '10px 16px' : '10px 28px 10px 32px', overflow: 'hidden',
              ...
              <div className="flex flex-col gap-2 w-full">
                      ...
                      return inst?.variations.map((v, idx) => {
                        ...
                        return (
                          <button key={idx} onClick={() => handleVariationChange(idx)}
                            ...
                            style={{ padding: '5px 10px', fontSize: '0.68rem', background: on ? 'linear-gradient(135deg, rgba(124,58,237,0.45), rgba(109,40,217,0.28))' : 'rgba(255,255,255,0.04)', border: on ? '1px solid rgba(168,85,247,0.55)' : '1px solid rgba(255,255,255,0.07)', color: on ? '#f0e0ff' : 'rgba(148,163,184,0.5)', boxShadow: on ? '0 0 10px rgba(124,58,237,0.3)' : 'none' }}>
                            {v}
                          </button>
                        )
                      })
                ...
                <div style={{ overflowX: 'auto', width: '100%' }}>
                  <Piano compact octaveStart={octave} numOctaves={2} keyboardMode={false}
                    ...
                    onNoteOn={handlePianoNoteOn} onNoteOff={handlePianoNoteOff} />
                </div>
              </div>
            ) : (
                ...
                <div className="flex items-start gap-3">
                  <div className="flex flex-col gap-2" style={{ overflowX: 'auto', flex: 1, minWidth: 0 }}>
                    <Piano octaveStart={octave} numOctaves={2} keyboardMode={keyboardMode}
                      highlightNotes={scaleHighlights} chordNotes={chordHighlights}
                      onNoteOn={handlePianoNoteOn} onNoteOff={handlePianoNoteOff}
                      onOctaveUp={handleOctaveUp} onOctaveDown={handleOctaveDown} />
                    <BassPedals octave={octave - 1} keyboardMode={keyboardMode}
                      highlightNotes={scaleHighlights} chordNotes={chordHighlights}
                      onNoteOn={handlePianoNoteOn} onNoteOff={handlePianoNoteOff} />
                  </div>
                      ...
                      return inst?.variations.map((v, idx) => {
                        ...
                        return (
                          <button key={idx} onClick={() => handleVariationChange(idx)}
                            ...
                            style={{ padding: '7px 10px', fontSize: '0.72rem', letterSpacing: '0.03em', textAlign: 'left',
          ...
          <div className="flex-1 flex flex-col overflow-hidden gap-3" style={{ padding: isMobile ? '12px 16px' : '12px 32px' }}>
            ...
            {/* Tab bar + Generate actions */}
            <div className={`flex-shrink-0 ${isMobile ? 'flex flex-col gap-2' : 'flex items-center gap-3'}`}>
              <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', ...(isMobile ? {} : { flexShrink: 0 }) }}>
                <div className="flex items-center gap-1"
                  style={{ background: 'rgba(10,5,22,0.85)', border: '1px solid rgba(124,58,237,0.2)', borderRadius: 14, padding: '4px', width: 'fit-content', backdropFilter: 'blur(12px)', boxShadow: '0 0 20px rgba(124,58,237,0.08)' }}>
                  {TABS.map(tab => {
                    ...
                    return (
                      <button key={tab} onClick={() => setActiveTab(tab)}
                        ...
                        style={{
                          ...
                          background: on ? 'linear-gradient(135deg, rgba(124,58,237,0.58), rgba(109,40,217,0.38))' : 'transparent',
              ...
              {activeTab === 'Generate' && (
                <div className="flex items-center gap-2 flex-shrink-0"
                  ...
                  <span style={{ fontSize: '0.68rem', color: 'rgba(148,163,184,0.4)', letterSpacing: '0.04em', flexShrink: 0 }}>
                    ...
                    {activeProg && <span style={{ color: 'rgba(34,211,238,0.5)', marginLeft: 4, fontSize: '0.6rem' }}>+ chords</span>}
                  </span>
                  <div className="flex items-center gap-1">
                    ...
                    {[1, 2, 4].map(b => (
                      <button key={b} onClick={() => setGenBars(b)}
                        ...
                        style={{ padding: '3px 8px', fontSize: '0.68rem', background: genBars === b ? 'rgba(124,58,237,0.45)' : 'rgba(255,255,255,0.04)', border: genBars === b ? '1px solid rgba(168,85,247,0.6)' : '1px solid rgba(255,255,255,0.07)', color: genBars === b ? '#e0aaff' : 'rgba(148,163,184,0.45)', boxShadow: genBars === b ? '0 0 8px rgba(124,58,237,0.3)' : 'none' }}>
                        {b}
                      </button>
                    ))}
                  </div>
                  <button
                    ...
                    style={{
                      ...
                      background: generating ? 'rgba(124,58,237,0.1)' : 'linear-gradient(135deg, rgba(124,58,237,0.58), rgba(109,40,217,0.38))',
                      ...
                      letterSpacing: '0.04em', textShadow: generating ? 'none' : '0 0 10px rgba(168,85,247,0.8)',
                    }}>
                    {generating ? (
                      <div className="w-3 h-3 rounded-full border-2 border-purple-500 border-t-transparent" style={{ animation: 'spin 0.8s linear infinite' }} />
                    ) : (
                      <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                        <polyline points="23,4 23,10 17,10"/><path d="M20.5 15a9 9 0 11-2.6-7.4L23 10"/>
                      </svg>
                    )}
                  ...
                  {melodies.length > 0 && (
                    <button onClick={playAll}
                      ...
                      style={{
                        ...
                        letterSpacing: '0.04em', textShadow: playAllIdx >= 0 ? '0 0 10px rgba(34,211,238,0.7)' : 'none',
                      }}>
                      {playAllIdx >= 0 ? (
            ...
            {/* Tab content — all 4 always mounted, CSS display for switching */}
            <div className="flex-1 overflow-hidden relative">
              ...
              <div style={{ display: activeTab === 'Generate' ? 'flex' : 'none', flexDirection: 'column', height: '100%', gap: 12 }}>
                <div className="flex-1 overflow-y-auto min-h-0">
                  ...
                  {/* ── Saved Loops collapsible ── */}
                  <div style={{ marginTop: 8, borderTop: '1px solid rgba(124,58,237,0.15)', paddingTop: 12 }}>
                    <button
                      onClick={() => setInventoryOpen(o => !o)}
                      className="flex items-center gap-2 w-full rounded-xl transition-all"
                      style={{ padding: '10px 14px', background: inventoryOpen ? 'rgba(124,58,237,0.1)' : 'rgba(255,255,255,0.02)', border: '1px solid rgba(124,58,237,0.2)', color: '#c084fc' }}>
                      ...
                      <span style={{ fontSize: '0.65rem', color: 'rgba(192,132,252,0.55)', background: 'rgba(124,58,237,0.2)', padding: '2px 8px', borderRadius: 8 }}>
                        {loops.length}
                      </span>
                      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"
                        style={{ marginLeft: 'auto', transform: inventoryOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
                        <polyline points="6,9 12,15 18,9"/>
                      </svg>
                    </button>
                    {inventoryOpen && (
                      <div style={{ marginTop: 10 }}>
                        <LoopInventory loops={loops} onDelete={handleDeleteLoop} onRename={handleRenameLoop} bpm={bpm} />
                      </div>
                    )}
                  </div>
                </div>
              ...
              {/* Chords */}
              <div style={{ display: activeTab === 'Chords' ? 'flex' : 'none', flexDirection: 'column', height: '100%' }}>
                <ChordProgressionPanel
                  root={root} scale={scale} bpm={bpm}
                  onChordChange={handleChordProgChange}
                  onActiveProgChange={handleActiveProgChange}
                />
              </div>
              ...
              {/* Browse */}
              <div style={{ display: activeTab === 'Browse' ? 'flex' : 'none', flexDirection: 'column', height: '100%' }}>
                <BrowseTab bpm={bpm} onSave={handleSaveMelody} root={root} />
              </div>
